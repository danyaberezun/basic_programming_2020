...
internal class mainTest {
    ...
    fun `filter should filter a full array`() {
        var x = listOf(0, 243, 2, 3, 4, 3, 3, 5, 3, 4, 2, 4234, 433, 22, -10)
        assertEquals(x.filter { it > 2 }, foo.filter(x, { it > 2}))
    }
    @Test
    fun `filter_r should filter a full array`() {
        var x = listOf(0, 243, 2, 3, 4, 3, 3, 5, 3, 4, 2, 4234, 433, 22, -10)
        assertEquals(x.filter { it > 2 }, foo.filter_r(x, { it > 2}))
    }

    @Test
    fun `countLines should count a full array`() {
        var x = listOf("Bla", "");
        assertEquals(listOf(3,0), foo.countLines(x))
    }
    ...
    fun `mapAccumL should return a sum of array`() {
        val y1 = listOf(9,6,3);
        val f1 = { x: Int, y: Int -> Pair(x, x*y)}
        assertEquals(Pair(5, listOf(45, 30, 15)), foo.mapAccumL(5, y1, f1))

        val y2 = listOf(2,4,8);
        val f2 = { x: Int, y: Int -> Pair(x+y, x*y)}
        assertEquals(Pair(19,listOf(10,28,88)), foo.mapAccumL(5, y2, f2))
    }
